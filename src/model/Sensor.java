...
public class Sensor {
	...
	/**
	 * Returns the probability for the sensor giving position (rX,rY) when the true position is (x,y).
	 * 
	 * Assumes that all positions are within the grid.
	 * 
	 * @param rX - x coordinate of wanted position
	 * @param rY - y coordinate of wanted position
	 * @param x - x coordinate of true position
	 * @param y - y coordinate of true position
	 * @param nbrS1 - number of positions one step from (x,y) ∈ {3,5,8}
	 * @param nbrS2 - number of positions two steps from (x,y) ∈ {5, 6, 7, 9, 11, 16}
	 * @return the probability of the sensor for the position (rX, rY) when actually in position (x, y)
	 */
